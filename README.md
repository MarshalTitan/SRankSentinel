...
3. Convert the supplied map coordinates to a local game-world destination and retain it across World Visit, teleport, zoning, and instance transitions. This feeds vnavmesh directly and is never a direct player-position write.
   After World Visit, native Teleport data is treated as a readiness snapshot: a transient empty/stale list receives a bounded settle window, then static territory candidates remain coordinate-ranked and are validated through bounded real Teleport submissions instead of being misreported as unattuned.
